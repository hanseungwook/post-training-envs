"""CIFAR-100 classification tasks."""
...
def _get_cifar100_train(subset_size: int):
    def fn():
        ds = torchvision.datasets.CIFAR100(_DATA_ROOT, train=True, download=True)
...
def _get_cifar100_test(subset_size: int):
    def fn():
        ds = torchvision.datasets.CIFAR100(_DATA_ROOT, train=False, download=True)
        X = torch.tensor(ds.data[:subset_size]).permute(0, 3, 1, 2).float() / 255.0
        y = torch.tensor(ds.targets[:subset_size])
        return X, y
    return fn
...
register_task(TaskSpec(
    ...
    max_train_time_s=240.0,
    train_subset_size=40_000,
    eval_subset_size=5_000,
    input_shape=(3, 32, 32),
    num_classes=100,
    output_description="class labels (N,) in [0, 99]",
    get_train_data=_get_cifar100_train(40_000),
    get_test_data=_get_cifar100_test(5_000),
    baseline_metric=0.01,
    tier=2,
    reference_code="""\
import torch
