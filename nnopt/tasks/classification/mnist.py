...
def _get_mnist_train(subset_size: int):
    def fn():
        ds = torchvision.datasets.MNIST(_DATA_ROOT, train=True, download=True, transform=T.ToTensor())
        X = ds.data[:subset_size].float().unsqueeze(1) / 255.0  # (N, 1, 28, 28)
        y = ds.targets[:subset_size]
        return X, y
    return fn


def _get_mnist_test(subset_size: int):
    def fn():
        ds = torchvision.datasets.MNIST(_DATA_ROOT, train=False, download=True, transform=T.ToTensor())
...
# Tier 1: MNIST with 10K params — trivial
register_task(TaskSpec(
    ...
    max_train_time_s=60.0,
    train_subset_size=10_000,
    eval_subset_size=2_000,
    input_shape=(1, 28, 28),
    num_classes=10,
    output_description="class labels (N,) in [0, 9]",
    get_train_data=_get_mnist_train(10_000),
    get_test_data=_get_mnist_test(2_000),
...
# Tier 2: MNIST with 2K params — forces efficient architecture
register_task(TaskSpec(
    task_id="mnist-2k",
    task_type="classification",
    dataset_name="MNIST",
